...
nestForm.prototype = {
  init(cfg) {
    ...
    this.listNames = 'customRouteDaysList';

    // 有数据的话 说明是编辑页面 否则的话 是新增页面
    this.isEditor = this.data && this.data.list && this.data.list.length;
    // 是编辑页面 
    if (this.isEditor) {
  ...
  initTab(value) {
    ...
    new AddTabs({
      ...
      initTabCallBack: function($this, title, index) {
        ...
        // 如果是编辑页面的时候
	if (self.isEditor) {
        // if (self.isEditor && title <=  self.data.list.length) {
          // 更新表单的值
    ...
    if (data.list && data.list.length) {
      ...
      if ($('.'+listNames+'-dayNum'+(index - 1)).length > 0) {
        if (item && (item.dayNum || (item.dayNum == 0))) {
          console.log(index)
          $('.'+listNames+'-dayNum'+(index - 1)).val(index);
        }
      }
  ...
  setFormElems($panel, title) {
    ...
    if (columns && columns.length) {
      ...
      var operateType;
      if (this.isEditor && title <=  this.data.list.length) {
        // 编辑页面
        operateType = 0;
      } else {
        // 新增页面
      ...
      var ihtml = '<input type="hidden" name="'+listNames+'['+(title - 1)+'].id" class="'+listNames+'-id'+(title - 1)+'"/>' + 
        ...
        '<input type="hidden" name="'+listNames+'['+(title - 1)+'].dayNum" class="'+listNames+'-dayNum'+(title - 1)+'" value="'+title+'"/>' +
  ...
  renderKindeditor($panel, title) {
    ...
    KindEditor.create(editorElem, {
      items: [
        ...
        uploadJson: this.uploadUrl,  // 该配置是上传图片的地址
